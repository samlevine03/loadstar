...
This document defines the product behavior and feature set for a best-in-class lifting tracker. It focuses on what the tracking experience should feel like to the lifter, what flows must exist, and what product decisions make logging faster, smarter, and more satisfying than typical weight-training apps.
...
Create the lifting equivalent of Strava, but with tracking as a truly first-class feature rather than a chore. A lifter should want to use the app in the middle of a workout because it helps them move faster, remember what they did last time, and stay locked in.
...
- The app predicts what is likely to happen next.
...
### 4. Templates Are Helpful, But Inference Is Better

Many users do not follow a perfectly static plan. The app should still feel personalized by inferring patterns from prior behavior rather than requiring rigid programming setup.

The product should support routines and plans, but the base logging experience must work even if the user never creates a formal template.
...
- "Help me remember what exercise comes next."
...
- Users frequently accept predicted next exercises or suggested defaults.
...
- Start empty session.
- Start from suggested routine based on recent history.
...
- Exercises commonly appearing next in sequence after the current exercise.
- Exercises from the most similar past session.
- Exercises from the user's inferred split/day pattern.

Suggested sections in the add-exercise UI:

- Suggested next
- Recent
- Usual for this session type
- Search results
...
- Custom exercises should support all normal logging behavior, history, suggestions, and progression views.
...
These are the features that can make the tracker feel truly special rather than merely usable.

### 1. Predict Next Exercise

After the user logs several exercises, the app should proactively suggest what likely comes next.

Signals:

- Sequence patterns from recent sessions.
- Similarity to previous workouts.
- Time-of-week and habitual schedule.
- Exercises commonly co-occurring in the same session.
- Whether the current exercise appears complete relative to prior patterns.

The UX should be assistive, not pushy:

- Show likely next exercises as tappable suggestions.
- Allow one-tap accept.
- Never block alternate choices.

### 2. Detect Session Type Without Formal Split Setup

Users should not need to declare "today is push day" for the app to behave intelligently.
...
This allows the product to say, in effect:

- "This looks like your usual push session."
- "Here are the exercises you typically do next."

The system should use actual user behavior, not a generic push/pull/legs template.

### 3. Pre-Build Likely Session Skeletons

As a user logs, the app can quietly prepare likely follow-up exercises and defaults based on similar prior sessions.
...
It should not require the user to manage explicit templates unless they want to.
...
- Add or switch to the next exercise.
...
- Suggest likely next exercises from history.

### Strong V1.5 Features

- Inferred session-type suggestions.
- Repeat a similar past workout.
- Rest timer integrated into logging flow.
- Lightweight set tags like warmup/backoff/failure.
- Suggested targets based on recent performance.
- Smarter ordering of exercise suggestions based on session sequence.

### Later Enhancements

- Superset/circuit grouping.
- Coach-program overlays.
...
### Decision 4: Suggestions Should Be Personalized, Not Generic

Recommendation:

- Derive intelligence from the user's own session history first.

Users will trust suggestions that reflect their actual habits more than generic fitness-program assumptions.
...
- Keep suggestions clearly labeled as suggestions.
...
2. The app already knows what they are probably about to do.
...
That combination makes the tracker feel like a training partner rather than an obligation.
